...
class GameState():
	...
	def printer(self, clear = 'no'):
		if clear == 'clear':
			clear_output()
		print('--------------') 
		print(np.array(range(1,8)).reshape(1,7,))
		current_board = np.copy(self.board)
		current_board = current_board.reshape(6,7)
		current_board[current_board == -1] = 2
		print(current_board)
		#for r in range(6):
			#print([1 if self.pieces[str(x)] == 'X' else x for x in self.board[7*r : (7*r + 7)]])
