...
def playMatchesBetweenVersions(env, run_version, player1version, player2version, EPISODES, logger, turns_until_tau0, goes_first = 0):
    ...
    if player1version == -1:
        player1 = User('player1 (Human)', env.state_size, env.action_size)
    else:
        ...
        player1 = Agent(f'player1(ver:{player1version})', env.state_size, env.action_size, config.MCTS_SIMS, config.CPUCT, player1_NN)

    if player2version == -1:
        player2 = User('player2 (Human)', env.state_size, env.action_size)
    else:
        ...
        player2 = Agent(f'player2(ver:{player2version})', env.state_size, env.action_size, config.MCTS_SIMS, config.CPUCT, player2_NN)
...
def playMatches(player1, player2, EPISODES, logger, turns_until_tau0, memory = None, goes_first = 0):
    ...
    for e in range(EPISODES):
        ...
        if isinstance(player1, User):
			#print("Playing against User: 1")
            env.gameState.printer()
        ...
        while done == 0:
            ...
            env.gameState.render(logger)
            if isinstance(player1, User):
		#print("Playing against User: 2")
                env.gameState.printer(clear='clear')
                action_to_column = action % 7 + 1
                print(f"column {action_to_column} was played")
